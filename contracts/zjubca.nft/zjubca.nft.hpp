...
CONTRACT[[eosio::contract("zjubca.nft")]] nft : public eosio::contract {
 public:
  ...
  ACTION issue(name to, asset quantity, vector<string> uris, string name,
               string memo);

  ACTION transferid(name from, name to, id_type id, string memo);
  ...
  TABLE token {
    ...
    asset value;       // token value, always 1
  ...
  using currency_index = eosio::multi_index<
      "stat"_n, stats,
      indexed_by<"byissuer"_n,
                 const_mem_fun<stats, uint64_t, &stats::get_issuer> > >;
