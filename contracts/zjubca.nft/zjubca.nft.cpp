...
ACTION nft::issue(name to, asset quantity, vector<string> uris, string tkn_name,
                  string memo) {
  ...
  eosio_assert(quantity.amount == uris.size(),
               "mismatch between number of tokens and uris provided");
...
ACTION nft::transferid(name from, name to, id_type id, string memo) {
  ...
  eosio_assert(send_token != tokens.end(),
               "token with specified ID does not exist");

  // Ensure owner owns token
  eosio_assert(send_token->owner == from,
               "sender does not own token with specified ID");

  const auto& st = *send_token;

  auto symbol_name = st.value.symbol.code().raw();
  currency_index currency_table(_self, symbol_name);
  auto currency = currency_table.find(symbol_name);
  eosio_assert(currency != currency_table.end(),
               "token with symbol does not exist. create token before issue");
  ...
  tokens.modify(send_token, same_payer, [&](auto& token) { token.owner = to; });

  // Change balance of both accounts
  sub_balance(from, st.value);
  add_balance(to, st.value, currency->issuer);
}

// Transfer a non-fungible token. The param `quantity.amount` is always 1.
ACTION nft::transfer(name from, name to, asset quantity, string memo) {
  // Ensure authorized to send from account
  eosio_assert(from != to, "cannot transfer to self");
  require_auth(from);
  ...
  eosio_assert(quantity.amount == 1,
               "cannot transfer quantity, not equal to 1");
  ...
  SEND_INLINE_ACTION(*this, transferid, {from, "active"_n},
                     {from, to, id, memo});
}

void nft::mint(name owner, name ram_payer, asset value, string uri,
               string tkn_name) {
...
ACTION nft::setrampayer(name payer, id_type id) {
  ...
  eosio_assert(payer_token != tokens.end(),
               "token with specified ID does not exist");

  // Ensure payer owns token
  eosio_assert(payer_token->owner == payer,
...
void nft::add_balance(name owner, asset value, name ram_payer) {
  ...
  if (to == to_accounts.end()) {
    ...
    to_accounts.modify(to, same_payer, [&](auto& a) { a.balance += value; });
  }
}
...
void nft::sub_supply(asset quantity) {
  ...
  currency_table.modify(current_currency, same_payer,
